...
class MultiFidelityEnv(object):
    ...
    def _fps_func(self,sleep_time,fps_env,fps_stop_event):
        while True:
            if not fps_stop_event.is_set(): break
    ...
    def search_policy(self,step_number,multi_step = 2,back_number = 1):
        MAX_STEP = 1e4
        ...
        while not search_finish:
            ...
            self.backend.step(step_number * multi_step)
            self.step_number+=1
        ...
        return action_list,action_index_list 
    ...
    def close(self):
        if self.fps_stop_event is not None:
            self.fps_stop_event.set()
        #print('MFENV close')
        self.backend.close()